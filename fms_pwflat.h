// fms_pwflat.h - piecewise flat right-continuous forward curve determined by points (t[i], f[i]) and extrapolated value _f.
/*
	...
	{
		{
			...
			//static_assert(std::isnan(forward(-1., 3, t, f)));
			...
			//static_assert(std::isnan(forward(t[2] + 1, 3, t, f)));
	...
	{
		{
			static constexpr double t[] = { 1,2,3 };
			static constexpr double f[] = { 4,5,6 };
			static_assert(math::isnan(integral(-1., 3, t, f)));
