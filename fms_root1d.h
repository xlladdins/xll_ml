...
namespace fms::root1d {
	...
	template<class X>
	constexpr X bracket(X x, X x0, X a = -fms::math::infinity<X>, X b = fms::math::infinity<X>)
	{
		if (a >= b || a >= x0 || x0 >= b) {
			return fms::math::NaN<X>;
		}
	...
	static_assert(math::isnan(bracket<double>(1, 3, 4, 2)));
	static_assert(math::isnan(bracket<double>(1, 1, 2, 4)));
